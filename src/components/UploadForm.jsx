...
export default function UploadForm({ onUploaded }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (passkey !== PASSKEY) {
      setError(
        "Incorrect passkey. Email owner abhishekverma.dev@yahoo.com for permission."
      );
      return;
    }
    if (!file || !prompt) {
    ...
    <div className="max-w-md mx-auto mt-8">
      <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <input
            type="password"
            placeholder="Enter passkey to upload"
            value={passkey}
            onChange={(e) => setPasskey(e.target.value)}
            className="border border-gray-300 rounded-lg p-3 focus:outline-none focus:border-blue-400 text-black"
            autoComplete="off"
          />
          <div
